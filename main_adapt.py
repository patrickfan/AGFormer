...
if __name__ == "__main__":
    ...
    checkpoint_time = "202507101845_p"
    load_pretrain = False  # [True, False] - Load pretrained encoder weights
    iter_graph = True  # [True, False]
    
    # Adaptive refinement parameters - replaced fixed values with lists
    gat_thresholds = [0.1, 0.2, 0.3, 0.3]  # GAT attention thresholds for each refinement
    refine_frequencies = [1, 2, 3, 4]   # Epochs at which to perform refinement
    ...
    graph_cfg = "k5_config3" # [config1, config2, config3]
    graph_path = os.path.join(data_path, 'graph')
    ...
    if iter_graph:
        print(f"GAT attention thresholds: {gat_thresholds}")
        print(f"Graph refinement frequencies (epochs): {refine_frequencies}")
    ...
    if not load_checkpoint:
        edge_tracking_data = {
            ...
            'refinement_parameters': {
                ...
                'gat_thresholds': gat_thresholds,
                'refine_frequencies': refine_frequencies
            }
        }

    for epoch in range(1, 11):
        # Check if this epoch is in the refinement frequencies list
        refinement_index = None
        if iter_graph and epoch in refine_frequencies:
            refinement_index = refine_frequencies.index(epoch)
            collect_att = True
        else:
            collect_att = False
        ...
        refinement_info = ""
        if collect_att:
            # Get the corresponding threshold for this refinement
            current_threshold = gat_thresholds[refinement_index]
            print(f"\t Aggregating attention weights for graph refinement (threshold: {current_threshold})...")
            ...
            if avg_attention_weights:
                ...
                refined_edge_indices = refine_edge_index_by_attention(base_edge_indices, avg_attention_weights, current_threshold, num_nodes)
    ...
    if iter_graph:
        evaluation_info += f"GAT Attention Thresholds: {gat_thresholds}\n"
        evaluation_info += f"Graph Refinement Frequencies: {refine_frequencies}\n"
